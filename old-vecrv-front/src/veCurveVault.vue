<template lang="pug">
  div(v-if="isDrizzleInitialized")
    p
      h2 Yearn veSDL "Backscratcher" Vault
      p
        div.green Earn <strong>{{ vault_boost - 1 | toPct(1) }}</strong> more weekly rewards staking with Yearn.
      p This vault accepts SDL in exchange for perpetual claim on Curve DAO admin fees across all Yearn products.
      p Since it locks SDL in Curve Voting Escrow for 4 years and regularly prolongs the lock, this vault doesn't have withdrawal functionality.
    p
      div 🦍 user
      div wallet balance: {{ crv_balance | fromWei(2) }} SDL
      div vested balance: {{ vested_balance | fromWei(2) }} SDL
      div gauges claimable: {{ gauge_balance | fromWei(2) }} SDL
      div vault balance: {{ vault_balance | fromWei(2) }} agveSDL
      div claimable rewards: {{ claimable | fromWei(2) }} ETH/SDL 🍣
    p
      div 🧮 vault
      ...
      div total supply: {{ vault_supply | fromWei(2) }} agveSDL ({{ vault_supply / total_vesdl | toPct(3) }} of total)
      div yearn vecrv: {{ agg_vecrv | fromWei(2) }} agveSDL ({{ agg_vecrv / total_vesdl | toPct(3) }} of total)
      div total vecrv: {{ total_vesdl | fromWei(2) }} agveSDL
    p
      div 🕹 interact
      div account: {{ username || activeAccount }}
      p.muted deposit SDL from wallet into agveSDL vault
      button(:disabled='has_allowance_vault', @click.prevent='on_approve_vault') {{ has_allowance_vault ? 'vault approved' : 'approve vault' }}
      button(:disabled='!has_allowance_vault', @click.prevent='on_deposit') deposit {{ crv_balance | fromWei(2) }} SDL
      button(@click.prevent='on_claim') claim {{ claimable | fromWei(2) }} 3Crv
    p.row
      p.muted deposit SDL from wallet, vesting and gauges into agveSDL vault
      button(:disabled='has_allowance_zap', @click.prevent='on_approve_zap') {{ has_allowance_zap ? 'SDL zap approved' : 'approve SDL zap' }}
      button(:disabled='minting_allowed', @click.prevent='on_approve_minter') {{ minting_allowed ? 'minter approved' : 'approve minter' }}
      button(:disabled='!has_allowance_zap || (need_minter && !minting_allowed)', @click.prevent='on_zap') zap {{ zap_balance | fromWei(2) }} SDL
    p.row
      div.muted
        div vault by 
...
export default {
  ...
  methods: {
    on_approve_vault() {
      //this.drizzleInstance.contracts['SDL'].methods['approve'].cacheSend(this.vault, max_uint, {from: this.activeAccount})
    },
    on_approve_zap() {
      //this.drizzleInstance.contracts['SDL'].methods['approve'].cacheSend(this.zap, max_uint, {from: this.activeAccount})
    },
    on_approve_minter() {
    ...
    async load_user_gauges() { // switch it up to use https://etherscan.io/address/0x99Cb6c36816dE2131eF2626bb5dEF7E5cc8b9B14#readContract
      //let pool_count = await this.drizzleInstance.contracts['CurveRegistry'].methods['getPoolsLength']().call()
      ...
      this.gauge_balance = 0//balances.reduce((prev, curr) => curr.add(prev))
  ...
  computed: {
    ...
    crv() {
      return this.drizzleInstance.contracts['SDL'].address
    },
    zap() {
    ...
    voter() {
      return this.drizzleInstance.contracts['SaddleSLPVoter'].address
    },
    vault_supply() {
      return this.call('veCurveVault', 'totalSupply', [])
    },
    crv_balance() {
      return this.call('SDL', 'balanceOf', [this.activeAccount])
    },
    three_crv_balance() {
      return this.call('SLP', 'balanceOf', [this.activeAccount])
    },
    vested_balance() {
    ...
    crv_allowance() {
      return this.call('SDL', 'allowance', [this.activeAccount, this.vault])
    },
    vault_balance() {
    ...
    vault_apy() {
      ...
      const virtual_price = 1//this.call('SLP', 'get_virtual_price', []) / 1e18
    ...
    agg_vecrv() {
      return this.call('SaddleVotingEscrow', 'balanceOf', [this.voter])
    },
    total_vesdl() {
      return this.call('SaddleVotingEscrow', 'totalSupply', [])
    },
    vault_rewards() {
      return this.call('SLP', 'balanceOf', [this.vault])
    },
    epochs_claimed() {
      return this.call('CurveRewardDistribution', 'user_epoch_of', [this.voter])
    },
    epochs_total() {
      return this.call('SaddleVotingEscrow', 'user_point_epoch', [this.voter])
    },
    has_allowance_vault() {
      return !this.call('SDL', 'allowance', [this.activeAccount, this.vault]).isZero()
    },
    has_allowance_zap() {
      return !this.call('SDL', 'allowance', [this.activeAccount, this.zap]).isZero()
    },
    has_allowance_y3crv_zap() {
      return !this.call('SLP', 'allowance', [this.activeAccount, this.y3crv_zap]).isZero()
    },
    need_minter() {
